...
async function setupApiMocks(page) {
  await page.route('**/api/**', async (route) => {
    ...
    if (pathName.includes('/roles')) return route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({
        success: true,
        data: {
          roles: mockRoles.roles,
          capabilities: mockRoles.capabilities,
          data: {
            roles: mockRoles.roles,
            capabilities: mockRoles.capabilities,
          }
    ...
    if (pathName.includes('/companies')) return route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({
        success: true,
    ...
    if (pathName.includes('/users')) return route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({
        success: true,
    ...
    if (pathName.match(/\/chargers\/\d+/)) return json(mockCharger);
    if (pathName.includes('/chargers')) return json([mockCharger]);
    ...
    if (pathName.includes('/tariffs')) return json([]);
    if (pathName.includes('/charge-groups') || pathName.includes('/chargeGroups')) return json([]);
    if (pathName.includes('/stations')) return json([{ id: 1, name: "Amsterdam Central Charging Hub", station_name: "Amsterdam Central Charging Hub" }]);
...
async function run() {
  console.log('🚀 Running focused subview capturer...');
  ...
  // 1. Charger Tabs: Profiles & Predictive
  ...
  const predictiveTab = page.locator('button[value="predictive"]').first();
  if (await predictiveTab.isVisible()) {
    await predictiveTab.click();
    await page.waitForTimeout(600);
    await takeShot(page, '15_Charger_Detail_PredictiveLoad_Tab.png');
  }
  ...
  const genBtn = page.locator('button:has-text("Generate Monthly Invoices")').first();
  ...
  const sepaDebitBtn = page.locator('button:has-text("SEPA Direct Debit (pain.008)")').first();
  ...
  await page.waitForSelector('button[value="users"]', { timeout: 10000 });
  ...
  await browser.close();
  console.log('🎉 All targeted subviews captured successfully!');
}
