...
export function SimulatorDigitalTwin({
  ...
  const getStatusBadge = () => {
    if (isCharging) {
      return (
        <Badge className="bg-cyan-500/20 text-cyan-300 border-cyan-500/30 px-3 py-1 font-mono animate-pulse">
          ⚡ CHARGING
        </Badge>
      );
    }
    if (isPreparing) {
      return (
        <Badge className="bg-amber-500/20 text-amber-300 border-amber-500/30 px-3 py-1 font-mono">
          🔌 PREPARING (PLUGGED)
        </Badge>
      );
    }
    if (isFaulted) {
      return (
        <Badge className="bg-rose-500/20 text-rose-300 border-rose-500/30 px-3 py-1 font-mono">
    ...
    if (isSuspended) {
      return (
        <Badge className="bg-orange-500/20 text-orange-300 border-orange-500/30 px-3 py-1 font-mono">
          ⏸️ {activeConn?.status.toUpperCase()}
        </Badge>
      );
    }
    return (
      <Badge className="bg-emerald-500/20 text-emerald-300 border-emerald-500/30 px-3 py-1 font-mono">
        🟢 AVAILABLE
      </Badge>
    );
  };
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-b from-[#1e2228] to-[#171a1f] p-6 shadow-2xl backdrop-blur-md">
      {/* Glow Effects */}
      <div
        className={cn(
          "absolute -top-32 -left-32 w-80 h-80 rounded-full blur-3xl pointer-events-none transition-all duration-700 opacity-20",
          isCharging
            ? "bg-cyan-500"
            : isFaulted
            ? "bg-rose-500"
      ...
      <div className="relative z-10 flex flex-wrap items-center justify-between gap-4 pb-5 border-b border-white/10">
        <div className="flex items-center gap-3">
          <div
            className={cn(
              "size-12 rounded-2xl flex items-center justify-center border shadow-lg transition-all duration-500",
              isCharging
                ? "bg-cyan-500/20 border-cyan-400/50 shadow-cyan-500/20"
                : isFaulted
                ? "bg-rose-500/20 border-rose-400/50 shadow-rose-500/20"
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] px-2 py-0.5 rounded-full bg-white/10 text-muted-foreground font-mono font-medium">
        ...
        <div className="flex items-center gap-3">
          {getStatusBadge()}
          <div className="flex items-center gap-2 px-3 py-1 rounded-xl bg-black/30 border border-white/10 text-xs font-mono">
            <span
              className={cn(
                "size-2 rounded-full",
                status === "connected"
          ...
          return (
            <button
              ...
              className={cn(
                ...
                isSel
                  ? "bg-[#54a8c7]/15 border-[#54a8c7]/50 shadow-md shadow-[#54a8c7]/10"
            ...
            >
              ...
              <div className="flex items-center gap-2">
                {c.isPlugged ? (
                  <span className="text-[10px] px-2 py-0.5 rounded-md bg-cyan-500/10 text-cyan-300 border border-cyan-500/20 flex items-center gap-1">
      ...
      <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-5">
        ...
        <div className="lg:col-span-5 flex flex-col justify-between p-5 rounded-2xl bg-black/40 border border-white/10 relative overflow-hidden">
          ...
          <div>
            ...
            {/* Visual Battery Bar */}
            <div className="relative w-full h-8 bg-black/60 rounded-xl p-1 border border-white/10 overflow-hidden mb-4">
              <div
                className={cn(
                  "h-full rounded-lg transition-all duration-500 relative flex items-center justify-end pr-2 font-mono text-[10px] font-bold text-black",
                  isCharging
                    ? "bg-gradient-to-r from-cyan-500 via-teal-400 to-emerald-400 shadow-lg shadow-cyan-500/20"
                    : isFaulted
              ...
              >
                {isCharging && (
                  <Zap className="size-3.5 text-black fill-black animate-bounce" />
                )}
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-2.5 mt-4 pt-4 border-t border-white/5 font-mono text-xs">
        ...
        <div className="lg:col-span-7 grid grid-cols-2 sm:grid-cols-3 gap-3">
          ...
          <div className="col-span-2 sm:col-span-3 p-4 rounded-xl bg-black/40 border border-white/10 flex flex-col justify-between">
          ...
          <div className="p-3.5 rounded-xl bg-black/40 border border-white/10 flex flex-col justify-between">
            <span className="text-[11px] font-bold text-muted-foreground flex items-center gap-1">
              <Zap className="size-3 text-amber-400" /> Voltage
            </span>
          ...
          <div className="p-3.5 rounded-xl bg-black/40 border border-white/10 flex flex-col justify-between">
            <span className="text-[11px] font-bold text-muted-foreground flex items-center gap-1">
              <ArrowDownUp className="size-3 text-cyan-400" /> Current
            </span>
            <div className="my-1.5">
              <span className="text-xl font-bold font-mono text-white">
          ...
          <div className="p-3.5 rounded-xl bg-black/40 border border-white/10 flex flex-col justify-between">
            <span className="text-[11px] font-bold text-muted-foreground flex items-center gap-1">
              <Thermometer className="size-3 text-rose-400" /> Thermals
            </span>
            <div className="my-1.5">
              <span
                className={cn(
          ...
          <div className="p-3.5 rounded-xl bg-black/40 border border-white/10 flex flex-col justify-between">
            <span className="text-[11px] font-bold text-muted-foreground flex items-center gap-1">
              <Layers className="size-3 text-emerald-400" /> Lifetime Wh
            </span>
            <div className="my-1.5">
          ...
          <div className="col-span-2 p-3.5 rounded-xl bg-black/40 border border-white/10 flex flex-col justify-between">
            <span className="text-[11px] font-bold text-muted-foreground flex items-center gap-1">
              <Radio className="size-3 text-purple-400" /> Authorized idTag
            </span>
            <div className="my-1.5 flex items-center justify-between">
              <span className="text-sm font-bold font-mono text-purple-300 truncate">
