/**
 * Zaptec (Go, Pro) Vendor Error Codes & 32-bit Hardware Status Bitmask
 * Serializes integer 32-bit hardware status bitmask into StatusNotification.vendorErrorCode
 */
...
export const ZAPTEC_FLAGS: Record<number, ZaptecBitmaskFlag> = {
  1: {
    ...
    action: "Soft reboot charger to release coil drive; if contacts remain welded, take EVSE offline and service relay.",
    severity: "CRITICAL",
  ...
  2: {
    ...
    action: "Disconnect vehicle cable; power cycle charger to re-zero DC current transducer.",
    severity: "CRITICAL",
    category: "PowerElectronics",
  ...
  256: {
    ...
    action: "Send dual UnlockConnector pulse to relieve mechanical pin tension; inspect socket lock pin path.",
    severity: "HIGH",
  ...
  65536: {
    ...
    action: "Derate charging current to 6A to allow passive heat dissipation; check ambient sun exposure.",
    severity: "HIGH",
    category: "Thermal",
  ...
  134217728: {
    ...
    action: "Inspect installation earthing and neutral conductor integrity; verify phase sequence.",
    severity: "HIGH",
    category: "GridFault",
...
/**
 * Decodes a Zaptec 32-bit hardware status integer into individual active flags
 */
...
export function getZaptecErrorInfo(code?: string | number | null): {
  ...
  if (!isNaN(num) && num > 0) {
    ...
    if (flags.length > 0) {
      // Prioritize CRITICAL severity flags
